...
def category_actuals(
    ...
    """Return category→actual expense totals for the current user/month."""
...
async def create_budget(
    ...
    month_start = date.fromisoformat(str(payload.month))
    ...
    actual = category_actuals(
        ...
        payload.month,
        (
            date(payload.month.year + 1, 1, 1)
            if payload.month.month == 12
            else date(payload.month.year, payload.month.month + 1, 1)
        ),
